  ...
  if (!isAuth) {
    return (
      <div className="flex-1 w-full animate-fade-in">
        {/* ─── HERO / LANDING PARA NO AUTENTICADOS ─── */}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-4 mb-10">
                  <button
                    ...
                    className="flex items-center gap-2.5 px-8 py-3.5 bg-primary text-white text-[13px] font-bold uppercase tracking-wider hover:bg-accent transition-all shadow-md"
                  >
                    <LogIn size={17} /> Iniciar Sesión
                  </button>
                  <button
                    onClick={() => { navigate('/login'); setTimeout(() => window.dispatchEvent(new Event('toggle-auth')), 100) }}
                    className="flex items-center gap-2.5 px-8 py-3.5 border-2 border-primary text-primary text-[13px] font-bold uppercase tracking-wider hover:bg-primary hover:text-white transition-all"
                  >
              ...
              <div className="hidden lg:flex flex-col items-center gap-6">
                <div className="relative flex items-center justify-center">
                  ...
                  <img
                    ...
                    className="w-[320px] h-auto object-contain relative z-10 drop-shadow-xl"
                  />
                </div>
          ...
          <div className="max-w-[1400px] mx-auto px-6 md:px-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {HERO_SLIDES.map((slide, i) => (
                <div
                  key={i}
                  className="relative group overflow-hidden shadow-lg animate-fade-in-up"
                  style={{
                ...
                >
                  ...
                  <div className="relative z-10 p-8 min-h-[320px] flex flex-col justify-end">
                    <img src={slide.image} alt={slide.tag} className="w-[60px] h-[60px] object-cover rounded-lg mb-4 shadow-lg block animate-fade-in"
                      style={{ animationDelay: `${i * 0.15 + 0.2}s`, animationFillMode: 'backwards' }}
                    />
                    <span className="inline-block w-max px-3 py-1 bg-white/20 backdrop-blur-md text-white text-[10px] font-bold uppercase tracking-[3px] mb-3 border border-white/20">
                      {slide.tag}
                    </span>
                    <h3 className="text-[22px] font-heading font-black text-white leading-tight mb-2 drop-shadow-lg">
                      {slide.title[0]}<br />{slide.title[1]}
                    </h3>
                    <p className="text-[13px] text-white/80 leading-relaxed font-medium drop-shadow-md">
                      {slide.sub}
                    </p>
                  </div>
                </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
          <div>
            ...
            <p className="text-[14px] text-text-muted font-medium flex items-center gap-2 mt-1">
              {user?.nombre}
              <span className={`inline-block px-2.5 py-0.5 text-[10px] font-bold text-white uppercase tracking-wider ${roleBadge}`}>
                {roleLabel}
              </span>
            </p>
          </div>
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 bg-white dark:bg-[#151522] border border-border-light/30 dark:border-white/10 shadow-sm p-6">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button onClick={() => navigate('/pedidos')} className="text-[12px] font-bold text-primary hover:text-accent transition-colors flex items-center gap-1">
                    Ver todos <ArrowRight size={14} />
                  </button>
                </div>
                {loading ? (
                  <div className="space-y-4">
                    {[1,2,3].map(i => <div key={i} className="h-12 bg-cream dark:bg-white/5 animate-pulse" />)}
                  </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left">
                      <thead>
                        <tr className="text-[11px] font-bold text-text-muted uppercase tracking-wider border-b border-border-light/30 dark:border-white/10">
            ...
            <div className="bg-white dark:bg-[#151522] border border-border-light/30 dark:border-white/10 shadow-sm p-6">
              ...
              <div className="flex items-center gap-3 p-4 bg-yellow-50 dark:bg-yellow-500/5 border border-yellow-200 dark:border-yellow-500/10">
                ...
                <p className="text-[13px] font-medium text-yellow-800 dark:text-yellow-400">
                  ...
                  {pendingOrders.map((o, i) => (
                    <div key={o.id || i} className="flex items-center justify-between p-4 bg-cream dark:bg-white/5 border border-border-light/30 dark:border-white/10">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 bg-blue-accent/10 flex items-center justify-center text-blue-accent">
                          <Package size={18} />
                        </div>
                        <div>
              ...
              <div className="bg-white dark:bg-[#151522] border border-border-light/30 dark:border-white/10 shadow-sm p-6">
                ...
                {recentOrders.length === 0 ? (
                  <div className="text-center py-10">
                    ...
                    <button onClick={() => navigate('/flores')} className="px-6 py-3 bg-primary text-white font-bold text-[12px] uppercase tracking-widest hover:bg-accent transition-all shadow-md">
                      Explorar catálogo
                    </button>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {recentOrders.map((o, i) => (
                      <div key={o.id || i} className="flex items-center justify-between p-3 bg-cream dark:bg-white/5">
                        <div>
                          <p className="text-[13px] font-bold text-text-dark dark:text-white">#{o.id || `ORD-${i+1}`}</p>
          ...
          <div className="relative">
            <div ref={scrollRef} className="flex gap-6 overflow-x-auto hide-scrollbar pb-4 snap-x snap-mandatory" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
              {products.map((product, i) => (
                <div key={product.id || i} className="min-w-[280px] max-w-[280px] flex-shrink-0 snap-start bg-white dark:bg-[#151522] border border-border-light/30 dark:border-white/10 group animate-fade-in-up"
                ...
                >
                  <div className="h-[260px] overflow-hidden bg-cream dark:bg-white/5 relative">
                    ...
                    {product.imagen ? (
                      <img src={product.imagen} alt={product.nombre} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center"><Flower2 size={48} className="text-border-light/50" /></div>
                    )}
                  </div>
                  <div className="p-5">
                    <span className="text-[10px] font-bold text-primary uppercase tracking-[2px]">{product.categoria || 'General'}</span>
                    <h3 className="text-[16px] font-heading font-bold text-text-dark dark:text-white mt-1 leading-tight truncate">{product.nombre || `Producto ${i + 1}`}</h3>
                    <div className="flex items-center justify-between mt-4">
                      <span className="text-[18px] font-heading font-black text-primary">Bs. {product.precio || '0.00'}</span>
                      <button onClick={handleOrderClick} className="px-4 py-2 bg-primary text-white text-[11px] font-bold uppercase tracking-wider hover:bg-accent transition-all">Pedir</button>
                    </div>
                  </div>
                </div>
              ))}
